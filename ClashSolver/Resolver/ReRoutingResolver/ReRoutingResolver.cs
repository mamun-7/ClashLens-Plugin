...
namespace ClashSolver.Resolver
{
    ...
    /// </summary>
    public class ReRoutingResolver
    {
        /// <summary>
        /// Revit Document.
        /// </summary>
        ...
        /// <summary>
        /// ElementIds of clashed element with target pipe
        /// </summary>
        private Collection<ElementId> m_clashIds;
		...
		public ReRoutingResolver()
        {
            m_rvtDoc = Application.GetUiApplication().ActiveUIDocument.Document;
        ...
        /// <param name="clashIds"> Pre-defined ElementId of clashed elements using filter of obstruction</param>
        public void Resolve(Pipe pipe, Collection<ElementId> clashIds = null)
        {
			//m_clashIds = clashIds;
        ...
        private void Filter(Pipe pipe, List<ReferenceWithContext> refs)
        {
            for (int i = refs.Count - 1; i >= 0; i--)
            {
                ...
                if (curElem.Id == pipe.Id || 
                    (!(curElem is Pipe) && !(curElem is Duct) && curElem.Category.BuiltInCategory != BuiltInCategory.OST_StructuralFraming) ||
                    (m_clashIds != null && !m_clashIds.Contains(curElem.Id))
                )
                {
                    refs.RemoveAt(i);
                }   
