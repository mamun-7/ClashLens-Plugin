...
{
	public class CeilOpeningResolver : BaseOpeningResolver
	{
		public CeilOpeningResolver()
		{
			HostType = OpeningHostType.Ceil;
		}
		...
		public override void CreateOpening()
		{
			...
			using (Transaction trans = new Transaction(Document, "Create Openings"))
			{
				...
				if(pipe is FamilyInstance familyInstance)
				{
					transform = familyInstance.GetTotalTransform();
				}
				else if(pipe.Location is LocationPoint locPoint)
				{
					transform = Transform.CreateTranslation(locPoint.Point);
				}
				else if(pipe.Location is LocationCurve locCurve)
				{
				...
				double width = CorrectLength(maxPoint.X - minPoint.X);
				double depth = CorrectLength(maxPoint.Y - minPoint.Y);
				...
				Opening opening = Document.Create.NewOpening(ceil, curveArray, true);
				trans.Commit();
			}
		}
